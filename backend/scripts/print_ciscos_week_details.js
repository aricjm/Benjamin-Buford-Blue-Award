...
function parsePick(line, week) {
  ...
  if (spaceOuMatch) {
    // If the first team is a known team and the second is a known team, or we can split them
    // Let's try to find a good split. Since we matched ([A-Za-z0-9&\s]+?), it might be greedy.
    // Let's check if we can split by looking at the words.
    // Actually, let's look at the specific lines:
    // "Neb Colorado O 58.5" -> team1: "Neb", team2: "Colorado"
    // "Auburn A&M o 50.5" -> team1: "Auburn", team2: "A&M"
    // Let's just split by space and take the first word as team1, and the rest as team2 (or vice versa).
    // For "Auburn A&M", team1 is "Auburn", team2 is "A&M".
    // For "Neb Colorado", team1 is "Neb", team2 is "Colorado".
    ...
    if (parts.length >= 4) {
      ...
      // Let's find the split point. Usually it's the first word vs the rest, or first two words vs the rest.
      // Let's check if the first word is a known team.
...
async function main() {
  const targetWeek = parseInt(process.argv[2]);
  if (isNaN(targetWeek)) {
    console.error('Please specify a week number.');
    process.exit(1);
  }

  const picks = parseAllPicks(PICKS_TEXT).filter(p => p.week === targetWeek);
  console.log(`=== WEEK ${targetWeek} BREAKDOWN ===`);
  ...
  for (const { week, pick, rawLine } of picks) {
    ...
    if (pick.type === 'spread') {
      let mappedTeam = TEAM_MAPPINGS[pick.team] || pick.team;
      if (week === 10 && pick.team === 'OSU') {
        // In Week 10, "Colorado/OSU" is Oregon State, but "OSU/Rut" is Ohio State.
        // Wait, this is a spread pick. Is there a spread pick for OSU in Week 10?
        // "OSU/Rut" is a total pick. "Colorado/OSU" is a total pick.
        // Wait, is there any spread pick for OSU in Week 10? No.
      }
      ...
      let mappedTeam = TEAM_MAPPINGS[pick.team] || pick.team;
      if (rawLine.includes('App state')) {
        console.log('DEBUG App state:', { pick, mappedTeam, week });
      }
    ...
    if (game) {
      ...
      if (pick.type === 'spread') {
        console.log(`- Pick: "${rawLine}"`);
        console.log(`  Picked: ${selectionTeam} ${pick.line > 0 ? '+' + pick.line : pick.line}. Game: ${game.away_team} (${game.score_away}) @ ${game.home_team} (${game.score_home}).`);
        console.log(`  Result: ${result.toUpperCase()}`);
      } else {
        console.log(`- Pick: "${rawLine}"`);
        console.log(`  Picked: ${pick.selection.toUpperCase()} ${pick.line}. Game: ${game.away_team} (${game.score_away}) @ ${game.home_team} (${game.score_home}). Total: ${Number(game.score_home) + Number(game.score_away)}.`);
        console.log(`  Result: ${result.toUpperCase()}`);
      }
    } else {
      console.log(`- Pick: "${rawLine}"`);
  ...
  console.log(`\nSummary: ${wins}W - ${losses}L - ${pushes}P`);
  await pool.end();
}
