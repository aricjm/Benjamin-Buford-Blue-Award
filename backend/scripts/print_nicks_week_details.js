  ...
  for (const { week, pick, rawLine } of picks) {
    ...
    if (pick.type === 'spread') {
      ...
      if (rows.length) {
        ...
        const isHome = selectionTeam === game.home_team;
        const selectionScore = isHome ? game.score_home + pick.line : game.score_away + pick.line;
        const opponentScore = isHome ? game.score_away : game.score_home;
        details = `Picked: ${selectionTeam} ${pick.line > 0 ? '+' : ''}${pick.line}. Game: ${game.away_team} (${game.score_away}) @ ${game.home_team} (${game.score_home}). Adjusted: ${selectionScore} vs ${opponentScore}.`;
      }
      ...
      if (rows.length) {
        ...
        const totalScore = game.score_home + game.score_away;
        details = `Picked: ${pick.selection === 'over' ? 'Over' : 'Under'} ${pick.line} for ${mappedTeam}'s game. Game: ${game.away_team} (${game.score_away}) @ ${game.home_team} (${game.score_home}). Total: ${totalScore}.`;
      }
    } else {
      ...
      if (rows.length) {
        ...
        result = determineTotalResult(game, pick.selection, pick.line);
        const totalScore = game.score_home + game.score_away;
        details = `Picked: ${pick.selection === 'over' ? 'Over' : 'Under'} ${pick.line}. Game: ${game.away_team} (${game.score_away}) @ ${game.home_team} (${game.score_home}). Total: ${totalScore}.`;
      }
    }
