...
// Team name mappings - shorthand to full names in the database
const TEAM_MAPPINGS = {
...
function parsePick(line) {
  ...
  if (spreadMatch) {
    ...
    return {
      team,
      type: 'spread',
      selection: spread > 0 ? 'away' : 'home', // selection side will be determined dynamically based on home/away status of the team
      line: spread
    };
  }
...
function parseAllPicks(text) {
  ...
  for (const line of lines) {
    if (line.startsWith('Week ')) {
      ...
      currentWeek = 16; // Bowl picks are in week 16 in the DB
...
function determinePickResult(game, selectionTeam, spread) {
  ...
  // spread is the value picked, e.g. +2 or -14.5
  // If we picked home team: homeScore + spread vs awayScore
  // If we picked away team: awayScore + spread vs homeScore
...
async function main() {
  ...
  for (const { week, pick, rawLine } of picks) {
    ...
    if (pick.type === 'spread') {
      ...
      // Query games for this week and season
  ...
  for (const week of sortedWeeks) {
    ...
    console.log(`Week ${week === 16 ? 'Bowl' : week}: ${stats.wins}W - ${stats.losses}L - ${stats.pushes}P (Total: ${stats.total})`);
  }
  console.log(`---------------------------------`);
  ...
  if (unmatched.length > 0) {
    ...
    unmatched.forEach(u => {
      console.log(`  Week ${u.week === 16 ? 'Bowl' : u.week}: "${u.rawLine}"`);
    });
  }
